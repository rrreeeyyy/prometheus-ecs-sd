...
IMPORT_PATH="github.com/rrreeeyyy/prometheus-ecs-sd"
...
GO_BUILD_LDFLAGS="-s -w -X ${IMPORT_PATH}/cmd.commitHash=${COMMIT_HASH} -X ${IMPORT_PATH}/cmd.buildDate=${DATE} -X ${IMPORT_PATH}/cmd.version=${VERSION}"
...
for OS in ${BUILD_PLATFORMS[@]}; do
  for ARCH in ${BUILD_ARCHS[@]}; do
    NAME="prometheus-ecs-sd-${OS}-${ARCH}"
    if [[ "${OS}" == "windows" ]]; then
      NAME="${NAME}.exe"
    fi
